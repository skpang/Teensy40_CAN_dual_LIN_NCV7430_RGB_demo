...
void setup() {
  ...
  pinMode(lin1_cs, OUTPUT);
  digitalWrite(lin1_cs, HIGH);
  pinMode(lin2_fault,INPUT);
  pinMode(lin2_cs, OUTPUT);
  digitalWrite(lin2_cs, HIGH);

  digitalWrite(LED_BUILTIN, HIGH);
  
  lin1.begin(&Serial3, 19200);
  lin2.begin(&Serial4, 19200);
  ...
  Serial.begin(115200);
  Serial.print("Teensy 4.0 Dual LIN NVC7430 RGB Demo. skpang.co.uk 11/23");
...
void init_ncv7430(void) {
  ...
  lin1.order(SET_LED_CONTROL, control_buffer, 4);
  lin2.order(SET_LED_CONTROL, control_buffer, 4);
}

void set_nvc7430_color(byte* message) {
  lin1.order(SET_LED_COLOUR, message, 8);
  lin2.order(SET_LED_COLOUR, message, 8);
}
